...
and the ungradable images for the quality classifier. Messidor-2 (ADCIS images, Krause et al. 2018 adjudicated grades, 1,744
gradable images / 874 patients) is frozen by `backend/data/freeze_external.py` as a second
external test — cached through the same Stage 0, checked for near-duplicates of everything the
grader saw (none), fingerprinted — and scored once by `backend/eval/score_external.py` at the
threshold locked on the calibration set; nothing is chosen on it. Protocol and results: `docs/VALIDATION.md` (generated by `backend/eval/write_docs.py`

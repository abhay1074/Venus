...
| `venus/stage3_explain.py` — `gradcam`, overlays, `attention_agreement` | `+drscreen/explain.m` | Grad-CAM in closed form for the GAP + sigmoid head: `ReLU(Σ_c w_kc A_c)` on the imported feature model's `top_activation` maps with the dense weights from `models/export/grader_v2_head.json` (identical to the gradient-tape maps, `tests/CrossCheckTest.m`); `imresize`, `labeloverlay`, `imdilate` for the lesion neighbourhood |

...
- **`unet`** when `backend/weights/lesion_unet.weights.h5` and `config/lesion_thresholds.json`
  exist: one U-Net (4 levels, 32 base filters, 512×512, four sigmoid channels MA/HE/EX/SE)
...
12,543 EyePACS images split by patient) is served when present; the label is the CNN's,
overridden to reject by the handcrafted hard limits, as the architecture specifies.
